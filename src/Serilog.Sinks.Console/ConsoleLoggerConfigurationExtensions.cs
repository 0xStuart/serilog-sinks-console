...
namespace Serilog
{
    ...
    public static class ConsoleLoggerConfigurationExtensions
    {
        ...
        /// </summary>
        /// <param name="sinkConfiguration">Logger sink configuration.</param>
        /// <param name="channelWriter">Threading Channel (LogEvent,byte[]) that is written to.</param>
        /// <param name="restrictedToMinimumLevel">The minimum level for
        /// events passed through the sink. Ignored when <paramref name="levelSwitch"/> is specified.</param>
        /// <param name="outputTemplate">A message template describing the format used to write to the sink.
        /// The default is <code>"[{Timestamp:HH:mm:ss} {Level:u3}] {Message:lj}{NewLine}{Exception}"</code>.</param>
        ...
        /// <exception cref="ArgumentNullException">When <paramref name="sinkConfiguration"/> is <code>null</code></exception>
        /// <exception cref="ArgumentNullException">When <paramref name="outputTemplate"/> is <code>null</code></exception>
        public static LoggerConfiguration ConsoleChannelWriter(
            this LoggerSinkConfiguration sinkConfiguration,
            Channel<(LogEvent,byte[])> channelWriter,
            LogEventLevel restrictedToMinimumLevel = LevelAlias.Minimum,
            string outputTemplate = DefaultConsoleOutputTemplate,
            IFormatProvider? formatProvider = null,
            LoggingLevelSwitch? levelSwitch = null,
            ...
            object? syncRoot = null)
        {
            ...
            return sinkConfiguration.Sink(new ConsoleChannelWriter(channelWriter,theme ?? SystemConsoleThemes.Literate, formatter, standardErrorFromLevel, syncRoot), restrictedToMinimumLevel, levelSwitch);
        }
